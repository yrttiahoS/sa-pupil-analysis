function pupilAnalysisCtrlNoise
...
%% parameters here
datatype = 'capetownMATLAB';%'tet300mus';%'tet300';% %'capetown';%'capetownMATLAB'; %'capetown';% 'capetownMATLAB'; % %boston, tampere, att
stimCodes = []; %can be used replace missing/erraneus values in gazedata (capetown)
ending = '.gazedata'; %.gazedata 
visualizeData = 0; % 0, 1
savevisualization = 1; % 0, 1
...
folder = [uigetdir() '\'];%'C:\Users\infant\Documents\MATLAB\igazelib\data - RGB control experiment\F2015 Ctrl\COntrolExpNoiseTest\';%'C:\Users\infant\Documents\MATLAB\igazelib\data - RGB control experiment\F2015 Ctrl\ControlExpMotherTEst\';%'C:\Users\infant\Documents\MATLAB\igazelib\SANFR toddler\pilot\'; %C:\Users\infant\Documents\MATLAB\igazelib\data - RGB control experiment\MOTHERs control exp\'%'C:\Users\infant\Documents\MATLAB\igazelib\MATLAB experiment SANFR\SAMother\'
...
min_pup_time = 300;%4000;%2800;%% 300;%
%end of time window for mean pupil diameter, ms; DONT USE VALUES > %5000!!!!!, %1200;1000; %5000;
max_pup_time = 1200;%5000;%3800;%1000; % ;1200; %1000 MUST BE MAX 1000 for 'FaceBorder' flash
...
if max_pup_time > 4000
    outputfilename = 'CtrlDilationNoise';
else
    outputfilename = 'CtrlConstrictionNoise';
end
...
if strcmp(datatype,'capetownMATLAB')
    plotMax = 480; % 8000 ms
elseif strcmp(datatype,'tet300mus')
        plotMax = 2000; % 2000 samples * (3.3 ms) =  6600 ms
...
comb_pup_waves = zeros(plotMax,1);
...
%     for j=1:1%length(files):length(files)
    ...
    elseif strcmp(datatype, 'tet300mus')
        [DATA, HEADERS] = loadGazeFile300mus(files{j});
    ...
    %writeGazedata2(HEADERS, DATA, files{j}, 'C:\Users\infant\Documents\MATLAB\igazelib\SA anonCTRL noise', 0, 0);  outputfilename = 'Etest4';
    ...
    for i=1:clipcount
        ...
        if length(whole_data{1}) < plotMax
            plotMax = length(whole_data{1});
        end
...
for i = 1:rc-1
    ...
    if combination_column(i) == 1 &&  ~isnan( comb_pup_waves(1,i))
...
try
    ...
    legendVector{end+1} = 'trig'
    %hleg1 = legend('SN','MN', 'SP', 'MP', 'trig');
    
    
    
    hleg1 = legend(legendVector);%'MN','MP', 'SN', 'SP', 'trig');
